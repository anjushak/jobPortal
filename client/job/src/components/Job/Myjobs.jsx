  ...
  const toggleJobStatus = async (jobId, currentStatus) => {
    ...
    if (job.disabled) {
      ...
      if (job.adminDisabled) {
          toast.error('This job is disabled by the admin and cannot be enabled.');
          return;
        }  
          ...
          jobs.map((job) => (
            <div className="jobsmain" key={job._id}>
              ...
              <div className="jobdetails">
               ...
               {job.disabled ? (
                  <FaEyeSlash onClick={() => toggleJobStatus(job._id, job.disabled)}  style={{fontSize:"19px",marginLeft:"10px",color:"red"}}/>
